...
### Introduction
This script allow you to convert a Strength Level CSV export to markdown files, one for each date. These markdown files can subsequently be imported into a note taking app, for instance. 
...
2. Run install.sh to create a virtual environment with the required python packages installed
```
chmod +x install.sh
./install.sh
```
